...
The map route is built from GPS in the EXIF of on-the-road photos, in three generated stages: photos → `scripts/extract-gps.mjs` → `scripts/gps-raw.json` → `scripts/build-track.mjs` → **`src/data/track.ts`** + `scripts/track.json` (ordered, thinned track) → `scripts/snap-roads.mjs` → **`src/data/route-geometry.ts`** (the track snapped onto real OSM roads via OSRM, plus `DAILY_KM`/`TOTAL_KM` real driven distance per day). All three files are **generated — do not hand-edit**. `Map.tsx` draws the driven portion (`ROUTE_GEOMETRY`) as a solid road line and the still-upcoming hotels/games as a dashed line, with a car icon at the start (SFO). Pre-trip SF hotels and games with a `reach` set (`"foot"`/`"transit"`/`"car"` — side-excursions from a base hotel; `"car"` draws a dashed spur base→stadium) get a marker but stay off the through-route line. The trip is ongoing: new photos and new legs are added incrementally — re-run all three stages (the snap step is cached per day). Full logic, tunables (`START_DATE`, `SEED`, `EXCLUDE`, `minKm`), and regenerate steps are in **`scripts/README.md`** — read it before touching the pipeline. The photo folder is not in the repo.
